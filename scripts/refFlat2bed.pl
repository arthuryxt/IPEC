#!/usr/bin/perl -w
use strict;
die "Usage: $0   \"input_refFlat\"    \"output_bed\"    \(optional\)\"track_name\"    \(optional\)\"erase_CDS > 0\"  \"\(optional\)add_\"chr\" > 0\" " if (@ARGV < 2);
my $filein1=$ARGV[0];
my $fileout=$ARGV[1];
my $track_name=$filein1;
if (scalar(@ARGV) > 2) { $track_name=$ARGV[2]; }
my $erase=0;
if (scalar(@ARGV) > 3) { $erase=$ARGV[3]; }
my $addchr=0;
if (scalar(@ARGV) > 4) { $addchr=$ARGV[4]; }
...
print OUT "track name=\"".$track_name."\" description=\"".$track_name."\"\n";
my $score=0;
my $col=0;
while (<IN>) {
    chomp;
    ...
    if (scalar(@a) eq 11) {
        print OUT join("\t",$a[2],$a[4],$a[5],$name,$score,$a[3],$a[6],$a[7],$col,$a[8],$lengths,$starts),"\n";
    }
    elsif (scalar(@a) eq 12) {
        my @b=split(/\,/,$a[11]);
